...
{    
	...
	tframe_queue_destory(&is->audioQueue);
	tframe_queue_destory(&is->videoQueue);
	SDL_DestroyMutex(is->transcode_mutex);
	SDL_DestroyCond(is->transcode_singal);
...
static int configure_audio_filters(VideoState *is, const char *afilters, int force_output_format)
{
	...
	av_buffersink_set_frame_size(is->out_audio_filter,
		1024);
...
void writeFrameQueue(AVFrame *frame,VideoState*is,bool audio)
{
#if 1
	TranscodeFrameQueue *queue = NULL;
	Frame * dstFrame = NULL;

	if (audio) queue = &(is->audioQueue);
	else       queue = &(is->videoQueue);

	if (!audio && AV_NOPTS_VALUE != frame->pts
		&& frame->pts == frame->pkt_pts){
		frame->pts = AV_NOPTS_VALUE;
	}
	dstFrame = tframe_queue_peek_writable(queue);
	if (!dstFrame) 
		return;

	av_frame_ref(dstFrame->frame, frame);

	SDL_LockMutex(is->transcode_mutex);
	tframe_queue_push(queue);
	SDL_UnlockMutex(is->transcode_mutex);
	SDL_CondSignal(is->transcode_singal);
#endif
}

void initOptionContext(VideoState*is)
{
	OptionsContext *optCtx = NULL;
	if (is->optctx) av_freep(&is->optctx);

	is->optctx = av_malloc(sizeof(OptionsContext));
	memset(is->optctx, 0, sizeof(OptionsContext));

	optCtx = is->optctx;

	optCtx->filename = "D:/haifeng.flv";
	optCtx->format = "flv";

	optCtx->video_disable = 0;
	optCtx->audio_disable = 0;
	optCtx->start_time = AV_NOPTS_VALUE;
	
	//audio
	optCtx->audio_channels = is->auddec.avctx->channels;
	optCtx->sample_rate = is->auddec.avctx->sample_rate;
	optCtx->sample_fmt = is->auddec.avctx->sample_fmt;
	optCtx->a_bitrate = 128 * 1024;
	optCtx->a_codec_id = AV_CODEC_ID_AAC;
	optCtx->a_dectb.den = is->auddec.avctx->time_base.den;
	optCtx->a_dectb.num = is->auddec.avctx->time_base.num;
	optCtx->a_pkttb = av_codec_get_pkt_timebase(is->auddec.avctx);

	//video
	optCtx->v_codec_id = AV_CODEC_ID_H264;
	optCtx->frame_rate=is->viddec.avctx->framerate;
	optCtx->frame_aspect_ratio=is->viddec.avctx->sample_aspect_ratio;
	optCtx->width=is->viddec.avctx->width;
	optCtx->height=is->viddec.avctx->height;
	optCtx->pix_fmt=is->viddec.avctx->pix_fmt;
	optCtx->v_bitrate=500*1024;
	optCtx->v_dectb.den = is->viddec.avctx->time_base.den;
	optCtx->v_dectb.num = is->viddec.avctx->time_base.num;
	optCtx->v_pkttb = av_codec_get_pkt_timebase(is->viddec.avctx);

	is->transcode_singal = SDL_CreateCond();
	is->transcode_mutex = SDL_CreateMutex();
	is->transcode_tid = SDL_CreateThread(transcode_thread, is);
...
static int audio_open(void *opaque, int64_t wanted_channel_layout, int wanted_nb_channels, int wanted_sample_rate, struct AudioParams *audio_hw_params)
{
	...
	audio_hw_params->channel_layout = av_get_default_channel_layout(audio_hw_params->channels);
...
//      of the seek_pos/seek_rel variables
        ...
        if (infinite_buffer<1 &&
	       ...
	       || ((is->audioq   .nb_packets > MIN_AUDIO_FRAME || is->audio_stream < 0 || is->audioq.abort_request)
...
static VideoState *stream_open(const char *filename, AVInputFormat *iformat)
{
	...
	if (tframe_queue_init(&is->audioQueue, 44))
		goto fail;
	if (tframe_queue_init(&is->videoQueue, 33))
		goto fail;
