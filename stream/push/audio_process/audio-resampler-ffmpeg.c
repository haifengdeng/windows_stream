...
{
	struct audio_resampler *rs = av_malloc(sizeof(struct audio_resampler));
	int errcode;
	...
	rs->output_planes = av_sample_fmt_is_planar(dst->format) ? rs->output_ch : 1;
	...
	if (errcode != 0) {
		av_log(NULL, AV_LOG_ERROR, "avresample_open failed: error code %d",
				errcode);
		audio_resampler_destroy(rs);
		return NULL;
	}
		 ...
		 const uint8_t *const input[], uint32_t in_frames)
{
	...
	*ts_offset = (uint64_t)swr_get_delay(context, 1000000000);
	...
	if (ret < 0) {
		av_log(NULL, AV_LOG_ERROR, "swr_convert failed: %d", ret);
		return false;
	}
