...
static void setMediaFilterClock(IGraphBuilder *graph)
{
	IMediaFilter *pMediaFilter = 0;
	IReferenceClock *pClock = 0;
	HRESULT hr = S_OK;
	hr = CoCreateInstance(&CLSID_SystemClock, 0, CLSCTX_INPROC_SERVER, &IID_IReferenceClock,(void**)&pClock);
	if (hr != S_OK){
		av_log(NULL,AV_LOG_ERROR, "failed to get system clock\n");
		goto End;
	}
	hr=IGraphBuilder_QueryInterface(graph,&IID_IMediaFilter, (void**)&pMediaFilter);
	if (hr != S_OK){
		av_log(NULL, AV_LOG_ERROR, "failed to get media filter\n");
		goto End;
	}
	hr=IMediaFilter_SetSyncSource(pMediaFilter,pClock);
	if (hr != S_OK){
		av_log(NULL, AV_LOG_ERROR, "failed to set system clock\n");
		goto End;
	}
End:
	if (pMediaFilter)
		IMediaFilter_Release(pMediaFilter);
	if (pClock)
...
AVInputFormat ff_dshowaa_demuxer = {
    .name           = "dshow_pingan",
    .long_name      = "DirectShow capture",
